...
window.SimModule = (function () {
  ...
  //
  // This is the one authoritative implementation of the trial step. sim-worker.js
  // carries a VERBATIM copy of this function and its small helpers below (it cannot
  // importScripts this module in every offline context, so the copies are kept
  // byte-identical on purpose). The inline copy in StrikeSim2040.html mirrors the
  // same math; parityHash() lets the app/tests assert all copies agree.
  //
  // It is fully self-contained: every config object (context, rng, teamResources,
  // settings, strikeMethods, redProfiles) arrives through `opts`. It does NOT read
  // any global, so it produces identical numeric output wherever it runs. The trial
  // math/semantics are unchanged from the existing engines — only the copies converge.
  //
  ...
  function simulateTrialCore(actionPlan, opts) {
    const o = opts || {};
    const context = o.context || buildSimContext();
    const rng = o.rng || createRng();
  ...
  // results into a stable 32-bit hash. The worker exposes the SAME computation over its
  // copy via a {type:'parity'} message, so the app/tests can assert inline === worker by
  // comparing hashes. Pure function of (seed, n): same inputs → same hash, every time.
  function parityFixture() {
    ...
    const opts = {
      ...
      // Inlined (not the module const) so the worker's verbatim copy is self-contained
      // and the two fixtures stay byte-identical for parity.
      redProfiles: {
        'Counter-Strike':     { pMult: 0.90 },
        'Cyber Defense':      { tag: 'cyber', pMult: 0.80 },
        'EW Countermeasure':  { tag: 'ew',    pMult: 0.80 },
  ...
  window.simulateTrialCore = simulateTrialCore;
