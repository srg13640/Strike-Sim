...
(function () {
  ...
  function assessment(score) {
    if (score >= 72 && state.metrics.escalation <= 58 && state.metrics.exposure <= 58) {
      return { tone: 'good', label: 'Campaign advantage', text: 'The plan credibly denies adversary objectives while preserving coalition endurance.' };
    }
    if (score >= 58) {
      return { tone: 'warn', label: 'Contested but recoverable', text: 'The campaign can still work, but one or more strategic constraints remains fragile.' };
    }
    return { tone: 'bad', label: 'Campaign failure risk', text: 'The force posture is likely too brittle for an extended contest.' };
  }
  ...
  function onClick(ev) {
    ...
    if (!t) return;
    if (t.hasAttribute('data-campaign')) {
      selectedCampaignId = t.getAttribute('data-campaign');
      render();
    } else if (t.hasAttribute('data-start')) {
      startCampaign();
    } else if (t.hasAttribute('data-action')) {
      toggleAction(t.getAttribute('data-action'));
    } else if (t.hasAttribute('data-resolve')) {
      resolvePhase();
    } else if (t.hasAttribute('data-reset')) {
      state = null;
      render();
    } else if (t.hasAttribute('data-export')) {
      exportBrief();
    } else if (t.hasAttribute('data-wargame')) {
      launchWarGameFromPosture();
    }
  }
