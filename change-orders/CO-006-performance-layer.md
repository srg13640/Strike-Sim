...
- Phase 3: ✅ COMPLETE (2026-07-11, this commit) — the war film and the debrief ceremony.
  - **WATCH war film:** `playWatch` opens with `watchCinematic` (letterbox in, comms floor returns) and `showOutcome` closes it with `watchDone` (bars out — also on SHOW RESULT NOW, which routes through the same function). Impact flash + expanding shockwave ring were already `flashStrike`'s (preserved); new per-event-class stingers ride the guarded `sfxA` bridge: `stingStrike` (film opens), `stingImpact` (hits), `stingKill` (two low falling notes — grim, not triumphant), `stingCascade` (a crackle stepping down the graph), plus the `tempoLoss` descending two-note motif, fired only when the shown tempo actually drops vs `op.lastTempoShown` (reset per operation). **Near-silence register resolved against the P2 contract:** the Phase 2 proof pinned "no WATCH bed exists"; Phase 3 keeps that contract — the war's score IS silence + sparse stingers + radio, no drone.
  - **Camera:** new `MapModule.flyToNode(nodeId, {zoom, duration})` — presentation-only (reads public lat/lon, no-ops when the map pane is hidden or under reduced motion). The Director throttles cuts on the deterministic pacing clock (`i * step`, CUT_SPACING_MS 1400, MAX_CUTS 8) and cuts only on kills/cascades. Kill confirmations land on the comms floor, Director-authored from resolved events and capped at 3/turn ('KILL CONFIRMED — ' / 'FRIENDLY UNIT DOWN — '), so the feed remains the record.
  - **AAR ceremony:** verdict stamps first under letterbox with the seed as serial ('after-action review · serial N'), silence (stopBed) beneath it; the ledgers deal in as cards (`.cin-deal` + per-card `--deal-i` stagger, soft tick each; class stripped by `setPhase` on exit; reduced-motion CSS lands cards fully set). Career rank rides the header as a chip computed with the same evidence-gated `analystRank` + seeded `bootstrapBss` band the calibration card uses (`'aar-rank-chip'` hash stream — display-only read). The kicker seed now reads as a serial plate (`.dir-serial`). EXIT TO CONSOLE letterboxes back to the title front door (`exitCinematic` → `openConsole`).
  - **Proof:** `performance-layer-proof.js` grew 21→31 contracts (film open/close pairing, stinger palette + guarded routing, cut discipline, flyToNode purity, sparse Director-authored confirms, tempo-motif honesty, ceremony structure, deal safety, exit path, frame budget: ≥110ms/event pacing floor, O(1) per-tick work, no layout reads in the hot loop, FX teardown). §4.6's 60 fps is held by construction on those bounds, per the runtime-performance-proof pattern (15/15 green). All CO-005 proofs green (doctrine 25, brier 29, mind-games 7/7, escalation, content-adaptation 17, director-ux, joint-force, validate-scenarios). `wargame-loop-eval` 8-match slice from seed-base 42 run twice: identical in every result field (only wall-clock `elapsed_ms` differs). Engine diff zero: git touched only StrikeSim2040.html (cache-busts), audio.js, cinematics.js, director.js (render layer), map.js (camera), and the proof.
- Phase 4: ✅ COMPLETE (2026-07-11, this commit) — W6 settings, the forced-motion architecture, and the full §4 sweep. **CO-006 CLOSED.**
  - **W6 settings store** (`strikesim.co006.settings`, cinematics-owned, cosmetic-only): reduced-motion toggle, performance mode, operator callsign, boot-fast toggle. The Settings panel gained the four controls under the audio sliders; everything live-applies and persists; the Phase-1 "arrive with Phase 4" IOU copy is gone.
  - **Forced reduced motion, one truth:** effective RM = system media query OR the toggle (`reduceMotion()` — the toggle can only ADD restraint, never disable a system preference). Cinematics publishes it as `html.cin-rm` and mirrors it into `AppShell.set({reducedMotion})` so shell ambient loops and engine.js's `fxReducedMotion()` (which prefers AppShell) inherit it for free. Every `@media (prefers-reduced-motion)` rule in the shell/director/map/cinematics gained an identical `html.cin-rm` twin; JS gates (`prefersReducedMotion()` in director, `flyToNode` + strike-FX in map, `playStrikeFX` in wargame.js) check the class first, matchMedia second. The load-time `reduceMotion` snapshot became a live call at all 15 sites — toggling mid-session takes effect immediately, no reload.
  - **Performance mode** (`html.cin-perf`): one dial to zero — the W1 scanline token is now actually CONSUMED (console frame boot/title scanlines ride `--fx-scanline-opacity`), and perf mode zeroes both FX dials, sets the three `--glow-*` tokens to none, strips text-shadows globally, hides the alert vignette. Structural box-shadows stay (legibility, not glow). Chosen scope note: glow/scanline/grain removal per §W6; panel glass shadows retained deliberately.
  - **Callsign in comms addressing:** sanitized at the boundary (uppercase, `A–Z 0–9 space dash`, ≤14 chars), read by the Director over the guarded bridge (`opAddr()`), applied to the three lines addressed TO the operator (OPERATION OPEN / PLANNING WINDOW OPEN / OPERATION CLOSED — "RAVEN 6, OPERATION OPEN — …"). Empty callsign = pre-P4 lines verbatim. The boot POST appends "OPERATOR OF RECORD" (after the fixed-index rows the data-link refresh targets). Cinematics still authors zero comms traffic.
  - **Boot honors the operator:** fast path = seen-flag AND bootFast; toggling it off replays the full POST every load; reduced motion still renders the log instantly regardless (dignity first).
  - **Mix/timing:** §5 grammar values were authored to spec in P1–P3 and verified against the tokens; no blind retuning without ears — deliberate no-change.
  - **§4 sweep:** (1) engine diff zero — git touched only StrikeSim2040.html/cinematics/director/map/wargame + the proof; (2) offline-complete static contracts green (no fetch/CDN, fonts vendored); (3) gesture-gated audio + persisted prefs (existing contracts); (4) `wargame-loop-eval` 8-match slice seed-base 42 run twice — byte-identical minus `elapsed_ms`; (5) reduced-motion path proven for BOTH the media query and the toggle; (6) 60 fps by construction (P3 frame-budget contracts; runtime-performance-proof green); (7) sole-author comms contracts green; (8) is Seth's to judge over a shoulder. Proof grew 31→39 contracts; all ten proofs green (doctrine, brier, mind-games 7/7, escalation, content-adaptation 17, director-ux 50, joint-force, runtime-performance, validate-scenarios, performance-layer 39).
  - Cache-busts: director/audio/cinematics → `co006-p4`, map → `p17`, wargame → `p13`.
