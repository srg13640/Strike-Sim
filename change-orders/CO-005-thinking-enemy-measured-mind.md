...
- REMAINING (final gate only): `tools/wargame-loop-eval.js` 200-seed run + `tools/wargame-loop-gate.js` against the 0.45–0.55 hard/hard target on the post-CO-005 code (eval is slow in the sandbox — run in background, poll, retune doctrine/policy constants only if the gate trips). All other §4 gate commands are green as of `371ee97`.
